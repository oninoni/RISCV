...
#open_checkpoint post_route_physopt.dcp

set_param tcl.collectionResultDisplayLimit 1024
#get a list of instance paths
#set proc_list [split [get_cells -hierarchical -filter { PRIMITIVE_TYPE=~ *bram.RAMB36* }] " "]0
#set proc_list [split [get_cells -hierarchical -filter { NAME=~ *instr_and_data_mem/RAM_reg }] " "]
set proc_list [split [get_cells -hierarchical -filter { NAME=~ *.bram }] " "]
...
append concatstring "    \<AddressSpace Name=\"core_" $i "_bram\" Begin=\"0\" End=\"1023\"\>"
puts $f $concatstring
set concatstring ""
puts $f "      \<BusBlock\>"
